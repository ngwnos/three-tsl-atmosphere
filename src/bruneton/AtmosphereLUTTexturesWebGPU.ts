...
import { reinterpretType, type AnyFloatType } from './core/types'
...
export class AtmosphereLUTTexturesWebGPU extends AtmosphereLUTTextures {
  ...
  invalidateComputeNodes(): void {
    this.transmittanceNode = undefined
    this.directIrradianceNode = undefined
    this.singleScatteringNode = undefined
    this.scatteringDensityNode = undefined
    this.indirectIrradianceNode = undefined
    this.multipleScatteringNode = undefined
  }
  ...
  computeMultipleScattering(
    ...
    this.multipleScatteringNode ??= Fn(() => {
      ...
      textureStore(
        this.scattering,
        globalId,
        texture3DLoad(scatteringRead, globalId, int(0)).add(vec4(luminance, 0))
      )
