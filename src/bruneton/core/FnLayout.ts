...
>({
  ...
  const fnCallback = (callback: unknown) =>
    callback as unknown as (args: unknown, builder: NodeBuilder) => unknown

  return typeOnly
    ? callback => Fn(fnCallback(callback))
    : callback =>
        Fn(fnCallback(callback)).setLayout({
          ...layout,
